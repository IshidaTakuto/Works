...
#define WEED_TEXTURE		"data/TEXTURE/weed.png"					// 草のテクスチャ
...
LPDIRECT3DTEXTURE9	CWeed::m_pTexture = NULL;
...
HRESULT CModelSetManager::Load(int nStage)
{
	...
	char *pFileName = CManager::GetSetFileName(nStage);
		...
		{// 終了メッセージまでループ
			...
			{// モデル配置情報
				...
				{// オブジェクトの生成
					...
					{// 丸太
						//CLog::Create(pos, rot, type, nType);
					}
				}
	...
	{// ファイルが開けない
		MessageBox(0, "ファイルがありません！", pFileName, MB_OK);
	}
...
HRESULT CWeed::Load(void)
{
	...
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, WEED_TEXTURE, &m_pTexture);
...
// 生成処理
//=============================================================================
CWeed* CWeed::Create(D3DXVECTOR3 pos)
{
	CWeed *pWeed = NULL;

	pWeed = new CWeed;	// メモリを確保
...
// コンストラクタ
//=============================================================================
CWeed::CWeed(int nPriority, CScene::OBJTYPE objType) : CScene3D(nPriority, objType)
{
	m_fMove = 0.0f;
	m_bMove = false;
}
...
// 初期化処理
//=============================================================================
HRESULT CWeed::Init(D3DXVECTOR3 pos)
{
	// 初期化処理
	CScene3D::Init();
	...
	// サイズの設定
	CScene3D::SetSize(D3DXVECTOR3(40.0f, 20.0f, 0.0f));
...
void CWeed::Update(void)
{
	...
	{// 右に動かす
		...
		pVtx[0].pos.x += m_fMove;
		pVtx[1].pos.x += m_fMove;
	}
	else
	{// 左に動かす
		m_fMove -= 0.02f;
	...
	float fWidth = CScene3D::GetWidth();

	if (m_bMove && -WEED_MOVE * 0.1f < pVtx[1].pos.x)
	{// 一定まで動いた
		m_bMove = false;
	}
	else if (!m_bMove && WEED_MOVE > pVtx[0].pos.x)
	{// 一定まで動いた
		m_bMove = true;
	}
