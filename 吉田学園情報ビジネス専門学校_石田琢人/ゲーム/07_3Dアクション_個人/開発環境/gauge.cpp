...
CGauge2D* CGauge2D::Create(int nNumBar, D3DXVECTOR3 pos, float fInit, float fMax, float fWidth, float fHeight, int nStartReduce)
{
	...
	{// メモリ確保成功
		...
		pGauge->m_pGauge = new CScene2D*[nNumBar];
		for (int nCntGauge = 0; nCntGauge < nNumBar; nCntGauge++)
		{// ゲージバーの生成
			pGauge->m_pGauge[nCntGauge] = CScene2D::Create();		// バーの生成
			pGauge->m_pGauge[nCntGauge]->SetPos(pos);				// 位置の設定
			pGauge->m_pGauge[nCntGauge]->SetSize(fWidth, fHeight);	// サイズの設定
		}
	}
...
// 値の設定
//=============================================================================
void CGauge2D::Set(float fGauge)
{
	m_fGauge = fGauge;
	...
	if (m_fGauge > 0)
	{// ライフが0より大きいとき
		fBarWidth = fWidth / (m_fMax / m_fGauge);	// バーの元の長さを100%としてライフを増減させる
	}
	else
	{// ライフが0のとき
		fBarWidth = 0;
...
// ゲージの増減
//=============================================================================
const void CGauge2D::AddSubtract(float fValue)
{
...
// 色の設定
//=============================================================================
const void CGauge2D::SetColor(D3DXCOLOR col, int nBar)
{
	if (m_nNumBar > nBar)
	{// 使われている本数まで
