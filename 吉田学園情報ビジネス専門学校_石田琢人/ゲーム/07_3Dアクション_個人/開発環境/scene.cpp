...
void CScene::StageUninit(void)
{
	...
	{// 優先順位分まわす
		...
		{// オブジェクトを先頭から消していく
			...
			if (OBJTYPE_ENEMY == pScene->m_objType || OBJTYPE_FIELD == pScene->m_objType || OBJTYPE_MODELOBJECT == pScene->m_objType || OBJTYPE_STAGELOGO == pScene->m_objType || OBJTYPE_WEED == pScene->m_objType)
			{// フェード以外の死亡フラグを立てる
				pScene->Uninit();					// 終了処理
			}
