...
void UpdateTutorial(void)
{
	...
	case TUTORIALSTATE_END:
		...
		{
			if (GetKeyboardTrigger(DIK_A) == true || GetKeyboardTrigger(DIK_LEFT) == true)
			{// ステート変更
				if (g_State != TUTORIALSTATE_NORMAL)
				{
					PlaySound(SOUND_LABEL_TURN);
					g_fCntMove -= 0.25f;
			...
			if (GetKeyboardTrigger(DIK_D) == true || GetKeyboardTrigger(DIK_RIGHT) == true || GetKeyboardTrigger(DIK_RETURN) == true)
			{// ステート変更
