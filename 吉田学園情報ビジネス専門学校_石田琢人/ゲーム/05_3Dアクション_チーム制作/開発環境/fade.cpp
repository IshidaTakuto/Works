...
STAGENUM				g_StageNext;
MODEFADE				g_modeGame;
bool					g_bFadeNow;				// フェード中
	...
	if (g_fade != FADE_NONE)
	{
		...
		else if (g_fade == FADE_OUT)
		{
			...
			if (g_colorFade.a >= 1.0f)
			{
				...
				if (g_modeGame == MODEFADE_MODE)
				{//モードの設定
					SetMode(g_modeNext);
				}
				else if (g_modeGame == MODEFADE_GAME)
				{//モード設定
					SetLoad();
				}
			}
