...
#define	TEXTURE_RANKING_BG			"data/TEXTURE/ranking_bg.png"		// 読み込むテクスチャファイル名

#define RANKING_TIME			(300)			// タイトル画面までの時間

#define MAX_TEXTURE				(3)				// テクスチャ数
#define RANKING_DIGIT			(7)				// 桁数
#define MAX_RANKING				(5)				// ランキング数

#define RANKING_X				(60.0f)			//数字の大きさ_X
#define RANKING_Y				(80.0f)			//数字の大きさ_Y
#define RANK_CHARACTER_X		(550)
#define RANK_CHARACTER_Y		(150)
...
void InitRanking(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, TEXTURE_RANKING_BG, &g_pTextureRanking[2]);
...
void UpdateRanking(void)
{
	...
	// 点滅用カウント
	nCntHighScore = (nCntHighScore + 1) % 4;
	...
	{// 画面遷移していない状態
		...
		{// ランキングスコアの数分回す
			...
			{// 今回取得したスコアがある
				for (int nCnt = 0; nCnt < RANKING_DIGIT; nCnt++)
				{// 数字を点滅させる
					if (nCntHighScore < 2)
					{// 赤色
					...
					{// 黒色
						g_nScoreCol[nCntRanking] = D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f);
					}

					// 頂点カラーの設定
					pVtx[(nCntRanking * RANKING_DIGIT * 4) + 0].col = g_nScoreCol[nCntRanking];
					pVtx[(nCntRanking * RANKING_DIGIT * 4) + 1].col = g_nScoreCol[nCntRanking];
					pVtx[(nCntRanking * RANKING_DIGIT * 4) + 2].col = g_nScoreCol[nCntRanking];
					pVtx[(nCntRanking * RANKING_DIGIT * 4) + 3].col = g_nScoreCol[nCntRanking];
...
void DrawRanking(void)
{
	...
	pDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, 4 * ((RANKING_DIGIT * MAX_RANKING) + 1), 2);
...
void MakeVertexRanking(LPDIRECT3DDEVICE9 pDevice)
{
	...
	// 頂点バッファを生成
	pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * RANKING_DIGIT * (MAX_RANKING + 1) + 1,
		D3DUSAGE_WRITEONLY,
		FVF_VERTEX_2D,
		D3DPOOL_MANAGED,
		&g_pVtxBuffRanking,
	...
	for (int nCntRanking = 0; nCntRanking < MAX_RANKING; nCntRanking++)
	{
		for (int nCntDigit = 0; nCntDigit < RANKING_DIGIT; nCntDigit++)
		{
			...
			{
				// 頂点カラーの設定
				pVtx[0].col = D3DXCOLOR(0.5f, 0.6f, 1.0f, 1.0f);
				pVtx[1].col = D3DXCOLOR(0.5f, 0.6f, 1.0f, 1.0f);
				pVtx[2].col = D3DXCOLOR(0.5f, 0.6f, 1.0f, 1.0f);
				pVtx[3].col = D3DXCOLOR(0.5f, 0.6f, 1.0f, 1.0f);
			}
			else
			{
				// 頂点カラーの設定
				pVtx[0].col = D3DXCOLOR(0.2f, 0.5f, 0.8f, 1.0f);
				pVtx[1].col = D3DXCOLOR(0.2f, 0.5f, 0.8f, 1.0f);
				pVtx[2].col = D3DXCOLOR(0.2f, 0.5f, 0.8f, 1.0f);
				pVtx[3].col = D3DXCOLOR(0.2f, 0.5f, 0.8f, 1.0f);
			}
