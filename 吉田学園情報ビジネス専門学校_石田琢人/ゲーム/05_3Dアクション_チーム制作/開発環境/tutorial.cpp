...
// マクロ定義
//*****************************************************************************
#define TUTORIAL_TEXTURENAME0	"DATA/TEXTURE/tutorial.jpg"	// タイトルのテクスチャ名
#define TUTORIAL_TEXTURENAME1	"DATA/TEXTURE/left.png"		// タイトルのテクスチャ名
#define TUTORIAL_TEXTURENAME2	"DATA/TEXTURE/right.png"	// タイトルのテクスチャ名
#define TUTORIAL_TEXTURENAME3	"DATA/TEXTURE/b.png"		// タイトルのテクスチャ名
#define MAX_TUTORIAL	(4)
#define LOGO_SIZE_X		(50)
...
void UpdateTutorial(void)
{
	...
	case TUTORIALSTATE_MOVE_L:
		...
		pVtx[1].tex.x = g_fCntMove + 0.5f;
		pVtx[2].tex.x = g_fCntMove;
		pVtx[3].tex.x = g_fCntMove + 0.5f;
	...
	case TUTORIALSTATE_END:
		...
		else if(pVtx[0].tex.x >= 0.5f)
		{// ステート変更
			g_State = TUTORIALSTATE_END;
		}
		else if(pVtx[0].tex.x == 0.5f || pVtx[0].tex.x == 0.5f)
		{// ステート変更
			g_State = TUTORIALSTATE_MIDDLE;
		}

		if (pVtx[0].tex.x >= 0.0f && pVtx[0].tex.x <= 0.5f)
		{
			if (GetKeyboardTrigger(DIK_A) == true || GetKeyboardTrigger(DIK_LEFT) == true || GetJoyPadTrigger(DIJS_BUTTON_21,0) == true || GetJoyPadTrigger(DIJS_BUTTON_13, 0) == true)
			{// ステート変更
			...
			if (GetKeyboardTrigger(DIK_D) == true || GetKeyboardTrigger(DIK_RIGHT) == true || GetKeyboardTrigger(DIK_RETURN) == true || GetJoyPadTrigger(DIJS_BUTTON_20, 0) == true || GetJoyPadTrigger(DIJS_BUTTON_12, 0) == true)
			{// ステート変更
