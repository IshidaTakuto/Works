...
void CScene::UpdateAll(void)
{
	...
	{// 優先順位分まわす
		...
		while (pScene != NULL)
		{// NULLでない限り回す
			//CDebugProc::Print("OBJ㍉%4d: %p\n", pScene->m_nID, pScene);	// デバッグ表示
	...
	//for (int nCntPriority = 0; nCntPriority < NUM_PRIORITY; nCntPriority++)
	//{// 優先順位分まわす
	//	CDebugProc::Print("TOP%d　　　　:　%p", nCntPriority, m_apTop[nCntPriority]);
	//	CDebugProc::Print("CUR%d　　　　:　%p", nCntPriority, m_apCur[nCntPriority]);
	//}

	//for (int nCntPriority = 0; nCntPriority < NUM_PRIORITY; nCntPriority++)
	//{// 優先順位分まわす
	//	CDebugProc::Print("PRIORITY%d: %d\n", nCntPriority, nCntPriorities[nCntPriority]);	// デバッグ表示
	//}
