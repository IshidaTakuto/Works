...
CMotionManager::TYPE *CMotionManager::m_pType[MAX_FILE] = {};				// モーションの種類
...
// １行分読み込む
//=========================================
char *CMotionManager::ReadLine(FILE *pFile, char *pDest)
{
	// 文字列を１行分取得
...
// 文字列を抜き出す
//=========================================
int CMotionManager::PopString(char *pSrc, char *pDest)
{
	int nCntString = 0;
	char *pStr = NULL;
...
HRESULT CMotionManager::Load(void)
{
	...
	for (nCntFile = 0; nCntFile < MAX_FILE; nCntFile++)
	{
		...
		{// ファイルがあった
			...
			{
				pStrCur = ReadLine(pFile, &aLine[0]);	// 文字列の先頭を読み込み
				...
				{// モーション数
					...
					m_pType[nCntFile] = new TYPE[m_nNumMotion[nCntFile]];
				...
				{// キャラクター情報の設定
					...
					{// 終了メッセージまでループ
						...
						else if (memcmp(aStr, "TYPE = ", strlen("TYPE = ")) == 0)
						{// モーションの種類
							pStrCur += strlen("TYPE = ");		// 頭出し
							strcpy(aStr, pStrCur);				// 文字列の取り出し

							m_pType[nCntFile][nCntMotion] = (TYPE)atoi(aStr);	// モーションの種類を保存
						}
						...
						{// 当たり判定
							...
							// モデル番号
							pStrCur += PopString(&pStrCur[0], &aStr[0]);
		...
		{// ファイルがないとき
			MessageBox(0, "ファイルがありません。", pFileName[nCntFile], MB_OK);
		}
	}
	return S_OK;
}
...
void CMotionManager::Unload(void)
{
	for (int nCntFile = 0; nCntFile < MAX_FILE; nCntFile++)
	{
		for (int nCntMotion = 0; nCntMotion < m_nNumMotion[nCntFile]; nCntMotion++)
		{
			...
			if (NULL != m_pFrame[nCntFile][nCntMotion])
			{// キーの破棄
...
// モーションの再生
//=========================================
void CMotion::SetMotion(int nMotion)
{
	if (m_nCurMotion != nMotion)
	{
		m_bLoop = m_pMotionManager->GetLoop(nMotion);			// ループの有無を取得
		m_nKey = m_pMotionManager->GetNumKey(nMotion);			// キー数を取得
		m_pKey = m_pMotionManager->GetKey(nMotion);				// キー情報を取得
		m_pFrame = m_pMotionManager->GetFlame(nMotion);			// フレーム数を取得
		m_type = m_pMotionManager->GetType(nMotion);			// モーションの種類を取得
		m_pCollision = m_pMotionManager->GetCollision(nMotion);	// 当たり判定
		m_pOrbit = m_pMotionManager->GetOrbit(nMotion);			// 軌跡
		m_nCurMotion = nMotion;									// モーション番号を記憶
...
// モーションクラスの生成
//=========================================
CMotion *CMotion::Create(int nFile, int nParts, CModel **pModel)
{
...
CMotion::CMotion(int nPriority, CScene::OBJTYPE objType) : CScene(nPriority, objType)
{
	...
	m_pModel = NULL;
	m_nCurMotion = 0;
	m_type = CMotionManager::TYPE_NEUTRAL;
...
void CMotion::Motion(void)
{
	...
	for (int nCntParts = 0; nCntParts < m_nParts; nCntParts++)
	{
		...
		{// モデルが使用されている
			...
			/*		X軸		*/
			// 角度の調節
			if (pKey->rotX > D3DX_PI)
			{
				pKey->rotX -= D3DX_PI * 2.0f;
			}
			if (pKey->rotX > D3DX_PI)
			{
				pKey->rotX += D3DX_PI * 2.0f;
			...
			// X目的の位置の調整
			if (rotModel.x > D3DX_PI)
			{
				rotModel.x -= D3DX_PI * 2.0f;
			}
			if (rotModel.x < -D3DX_PI)
			{
				rotModel.x += D3DX_PI * 2.0f;
			...
			/*		Y軸		*/
			// 角度の調節
			if (pKey->rotY > D3DX_PI)
			{
				pKey->rotY -= D3DX_PI * 2.0f;
			}
			if (pKey->rotY > D3DX_PI)
			{
				pKey->rotY += D3DX_PI * 2.0f;
			...
			/*		Z軸		*/
			// 角度の調節
			if (pKey->rotZ > D3DX_PI)
			{
				pKey->rotZ -= D3DX_PI * 2.0f;
			}
			if (pKey->rotZ > D3DX_PI)
			{
				pKey->rotZ += D3DX_PI * 2.0f;
			...
			// Z目的の位置の調整
			if (rotModel.z > D3DX_PI)
			{
				rotModel.z -= D3DX_PI * 2.0f;
			}
			else if (rotModel.z < -D3DX_PI)
			{
				rotModel.z += D3DX_PI * 2.0f;
			}
