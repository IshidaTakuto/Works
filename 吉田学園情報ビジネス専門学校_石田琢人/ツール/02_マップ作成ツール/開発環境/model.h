...
public:
	...
	int GetModel(void) { return m_nIdxModel; };						// モデル番号の取得
	const void SetModel(char *pModelName);							// モデルの設定

	CModel *GetParent(void) { return m_pParent; };					// 親の取得
	const void SetParent(CModel *parent) { m_pParent = parent; };	// 親の設定
