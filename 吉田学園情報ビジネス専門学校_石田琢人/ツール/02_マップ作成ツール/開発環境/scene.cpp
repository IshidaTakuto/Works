...
void CScene::RangeDelete(void)
{
	...
	{// 優先順位分まわす
		...
		{// NULLでない限り回す
			CScene *pSceneNext = pScene->m_pNext;	// 更新内で削除された時のために値を保持
			if (OBJTYPE_ENEMY == pScene->GetObjType())
			{// 敵
				CEnemy *pEnemy = (CEnemy*)pScene;
				pEnemy->Range();
			}
			else if (OBJTYPE_SETOBJ == pScene->GetObjType())
			{// 配置モデル
				CModelObject *pModelObj = (CModelObject*)pScene;
				pModelObj->Range();
			}
			pScene = pSceneNext;					// 次のオブジェクトを入れる
		}
...
void CScene::LoadDelete(void)
{
	...
	{// 優先順位分まわす
		...
		{// NULLでない限り回す
			...
			if (pScene->GetObjType() == OBJTYPE_ENEMY || pScene->GetObjType() == OBJTYPE_SETOBJ)
			{// 配置物を削除
				pScene->Uninit();	// 終了処理
			}
