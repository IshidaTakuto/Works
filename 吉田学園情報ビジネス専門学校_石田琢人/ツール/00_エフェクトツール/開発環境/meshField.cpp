...
// マクロ定義
//*****************************************************************************
#define TEXTURE_MESHFIELD_00		"data/TEXTURE/field000.jpg"	// 読み込むテクスチャファイル名

#define MESH_X			(6)			// x軸方向のマス数
#define MESH_Z			(6)			// z軸方向のマス数
#define MESH_WIDTH		(600.0f)		// 幅
#define MESH_DEPTH		(600.0f)		// 奥行
...
void InitMeshField(void)
{
	...
	// テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, TEXTURE_MESHFIELD_00, &g_pTextureMeshField[0]);
	...
	for (int nCntField = 0; nCntField < MAX_FIELD; nCntField++)
	{
		...
		g_MeshField[nCntField].nType = nCntField;										// フィールドのテクスチャ指定
	}
...
bool CollisionField(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld, D3DXVECTOR3 *pMove)
{
	...
	for (int nCntField = 0; nCntField < MAX_FIELD; nCntField++)
	{
		...
		{// 上から当たった時
			...
			pPos->y = g_MeshField[nCntField].pos.y;	// 埋まっているのを地面の上に戻す
